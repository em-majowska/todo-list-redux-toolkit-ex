...
export const fetch_list = createAsyncThunk("tasks/fetch_list", async () => {
...
export const create_task = createAsyncThunk(
  "tasks/create_task",
  async (title: string) => {
    const { data } = await axios.post<TTask>("http://localhost:3000/tasks", {
      title: title,
    });
...
export const delete_task = createAsyncThunk(
  "tasks/delete_task",
  async (id: string) => {
    const { data } = await axios.delete<TTask>(
...
const tasksSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(fetch_list.pending, (state) => {
      state.isLoading = true;
      state.error = null;
    });
    builder.addCase(fetch_list.rejected, (state, action) => {
    ...
    builder.addCase(create_task.rejected, (state, action) => {
      state.error = action.error.message || "Impossible to create a task";
      state.isLoading = false;
    });
    ...
    builder.addCase(delete_task.rejected, (state, action) => {
      state.error = action.error.message || "Impossible to delete";
      state.isLoading = false;
    });
...
export const todosSelector = (store: RootState) => store.tasksSlice;
